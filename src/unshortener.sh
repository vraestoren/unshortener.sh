...
params=null
vk_user_id=null
api="https://unshortener.divweb.ru/api"
...
function authenticate() {
    vk_user_id=$2
    params="vk_access_token_settings=${5:-}&vk_app_id=7631840&vk_are_notifications_enabled=${6:-0}&vk_is_app_user=${7:-0}&vk_is_favorite=${8:-0}&vk_language=${9:-ru}&vk_platform=${10:-desktop_web}&vk_ref=$4&vk_ts=$3&vk_user_id=$2&sign=$1"
    echo "$params"
}

# 1 - link: (string): <link>
function unshorten_link() {
    curl --request POST \
        --url "$api/link" \
        --user-agent "$user_agent" \
        --header "content-type: application/json" \
        --data "{\"link\":\"$1\",\"id\":$vk_user_id,\"params\":\"$params\"}"
}
